...
- **Card de Lideranças (`Liderancas.tsx`) — botão "Convidar"**: substitui o bloco antigo (código + 📋 inline). Botão verde-lima `[🔗 Convidar]` aparece ao lado de Editar/Maps/Excluir **só** quando `s.invite_code && !s.invite_used_at` (Opção A: esconde quando o código já foi consumido). Click abre `<InviteModal>` (`components/liderancas/InviteModal.tsx`) — modal centralizado via Radix Dialog primitive (mesmo padrão do `ConfirmDelete`) com: input read-only mostrando `${window.location.origin}/convite/${code}` + 📋 inline, e grid 2×2/4-cols com 4 canais — **WhatsApp** `#25D366` (`wa.me/?text=...`), **SMS** `#2563EB` (`sms:?body=...`), **E-mail** `#EA580C` (`mailto:?subject=...&body=...`), **Copiar Link** `#7C3AED` (clipboard + toast). Mensagem padrão personalizada pelo primeiro nome da liderança. Códigos já usados ficam com selo discreto âmbar "· convite usado" no rodapé do card (sem botão).
...
- **`NOTIFY pgrst, 'reload schema'` é FLAKY pra mudanças estruturais.** Para coluna nova + trigger + grant juntos (migration-042, 046), o NOTIFY às vezes não atualiza o cache do PostgREST e o frontend passa a ler 0 linhas silenciosamente (`SELECT *` falha por coluna desconhecida, `SupabaseCollection` zera o snapshot sem alertar na UI — erro só no console). **Fix garantido**: terminar conexões do role `authenticator`, o que força reconexão e re-leitura do schema: `SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE usename = 'authenticator' AND state = 'idle';`. Ou, mais simples: Dashboard → Settings → API → "Restart server". Aconteceu 2× nesta linha de trabalho (finance + hierarquia).
